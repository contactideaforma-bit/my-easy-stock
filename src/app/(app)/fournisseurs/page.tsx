  ...
  return (
    <div className="space-y-4 pb-8">
      <header className="flex items-center gap-3 pt-2">
        ...
        <h1 className="text-xl font-bold text-ink flex-1">Fournisseurs</h1>
      </header>

      <div className="glass p-1 grid grid-cols-2 gap-1">
        {(['commandes', 'fournisseurs'] as const).map((t) => (
          <button key={t} className={tab === t ? 'btn-primary !py-2 text-sm' : 'btn !py-2 text-sm text-ink/60'} onClick={() => setTab(t)}>
      ...
      {tab === 'commandes' && (
        <>
            ...
            <div className="glass-strong p-4 space-y-3">
              ...
              {hits.length > 0 && (
                <div className="space-y-1">
                  {hits.map((v) => (
                    <button key={v.id} className="w-full text-left text-sm text-ink py-1.5 px-2 rounded-xl hover:bg-white/5" onClick={() => addLine(v)}>
              ...
              {lines.map((l, i) => (
                <div key={l.variant.id} className="flex items-center gap-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-ink/55">{variantLabel(l.variant)}</p>
                  </div>
                  ...
                  <button className="text-rose-500/70" onClick={() => setLines(lines.filter((_, j) => j !== i))}>
              ...
              {lines.length > 0 && (
                <p className="text-right text-crystal-800 text-sm">Total : <span className="font-bold text-ink">{fmt(orderTotal)}</span></p>
              )}
          ...
          <div className="space-y-3">
            ...
            {purchases.length === 0 && <div className="glass p-8 text-center text-ink/55 text-sm">Aucune commande.</div>}
          </div>
        </>
      )}
    </div>
